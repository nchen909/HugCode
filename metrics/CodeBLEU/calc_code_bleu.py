...
# -*- coding:utf-8 -*-
import argparse
from metrics.CodeBLEU import bleu, weighted_ngram_match, syntax_match, dataflow_match
# import metrics.CodeBLEU.weighted_ngram_match
# import metrics.CodeBLEU.syntax_match
# import metrics.CodeBLEU.dataflow_match
...
def get_codebleu(refs, hyp, lang, params='0.25,0.25,0.25,0.25',args=None):
    ...
    if args:
        keywords_path=args.data_dir+"/../metrics/CodeBLEU/keywords/"
        keywords = [x.strip() for x in open(keywords_path + lang + '.txt', 'r', encoding='utf-8').readlines()]
    else:
        keywords = [x.strip() for x in open('/data/pretrain-attention/CodePrompt/metrics/CodeBLEU/keywords/' + lang + '.txt', 'r', encoding='utf-8').readlines()]
